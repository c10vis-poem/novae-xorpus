...
Repository Model: 5+1 Tier Cognitive Memory, OpenWiki TUI File Management, Multi-Model Cross-Auditing, Sandboxed Red Gatekeeping, and Distributed RAG Manifests
...
7. Official Enterprise Auditor vs. Incognito Red Gatekeeper:
   * The Cross-Auditor / Data Log Collector / Script Editor Agent serves as the sole recognized, public-facing auditing authority across the enterprise stack.
   * The Red Auditor operates strictly incognito within a sealed sandbox (.incognito_red_sandbox/), evaluating claims and trajectories behind a private hand-off without exposure to worker agents.
   * NOTE (repo correction, see project_red_auditor_not_detached.md): there is no standalone "Red Agent Auditor" entity — this two-role pair (Official / Red) is the entire model. A stale top-level `red-agent-auditor/` directory sourced from `PROPOSED_FILE_TREE.txt` exists elsewhere in this repo and should be removed as part of the broader restructure, not extended.
8. Universal Source of Truth: All local auditor nodes and the sandboxed Red Auditor must evaluate claims and script outcomes against the shared master memory bank and database (#d.u.m.b.a.s.s.).
...
│   ├── architectures/             # System blueprints, data flows, interface specs (*.md)
│   ├── entities/                  # Registries, schema contracts, hardware profiles (*.md)
...
│   └── kv_store/                  # Low-latency key-value entity lookups (*.db, *.json)
│
...
│   ├── extracted_tools/           # Deterministic tools mined from ingested documentation
│   │   ├── cli/                   # Extracted command-line utilities (*.sh)
│   │   └── wrappers/              # Extracted API interfaces & micro-functions (*.py)
│   ├── runtimes/                  # Master operational scripts and execution hooks (*.py, *.sh)
│   └── policies/                  # Validation guards, retry policies, schemas (*.json, *.yaml)
│
└── 05_episodic_logs/              # TIER 5: TELEMETRY & EPISODIC RUNS (Access: APPEND-ONLY)
    ...
    ├── trajectories/              # Multi-model traces: Query, Executor, Frontier (*.jsonl)
    ├── .incognito_red_sandbox/    # [SHADOW ISOLATION] Red Auditor evaluation reports & pass/fail quarantine (*.jsonl)
    ├── rlvr_verifiers/            # Graded rewards, assertion outcomes (+1.0 / -1.0) (*.jsonl)
    └── hygiene_reports/           # Schema integrity audits, broken link checks (*.md)
```

NOTE (repo correction, see project_novae_xorpus_repo_layout_correction.md): "04_skills_runtime/" here is Document 05's own internal framing and is one of the two "single shared skills/tools folder" instances the user flagged as stale. Skills/tools placement in this repo follows the corrected rule: each skill or tool lives in whichever specific repo actually uses it, not centralized here by default.
...
```
    ...
    • Edge-case recovery rules      • Data conversion routines
                │                               │
                ▼                               ▼
      [ owning repo's skills/tools location, per project_novae_xorpus_repo_layout_correction.md — not a single shared 04_skills_runtime/ ]
```

### Extraction Classification Rules
1. Targeting Skills: If a passage describes how to reason, how to structure an analysis, or how to coordinate a sequence of tasks, format it as a markdown skill with YAML frontmatter and place it in the repo that owns that skill's use.
2. Targeting Tools: If a passage contains executable code, CLI command flags, API request formats, or data transformation utilities, extract it into an executable script (.py or .sh) with explicit argument parsing, placed in the owning repo.
3. Registration: Update that repo's local manifest.jsonl immediately upon extraction.
...
└─────────────────────────────────────────────┬──────────────────────────────────────────┘
                                              ...
                                              ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ HOME NODE: Sandboxed Red Auditor Gatekeeper (.incognito_red_sandbox/)                  │
│ - Operates strictly incognito inside a sealed execution sandbox.                       │
...
* Dispositions:
   ...
   * VERDICT_REJECTED (-1.0): Logged to 05_episodic_logs/.incognito_red_sandbox/ with an explicit failure reason, triggering recursive self-correction in Tier 4 policies.

Open flag (from `reviews_and_audits/REVIEW-CANON-AND-BUILDERS-GUIDE.md`, not yet resolved by the user): whether `.incognito_red_sandbox/` should remain a dot-folder inside vault_root at all, versus moving it out entirely and exposing only a `red_verdict: pass|fail|n/a` field on trajectory records.
...
│ - Updated Prompt Skills ──► owning repo's skills location              │
│ - Updated Extracted Tools ──► owning repo's tools location             │
...
│ - Load skills from the owning repo     │
...
* Step 2: Scaffolding or Modifying Repo Content (Write Pass):
   ...
   3. Extract procedural skills and executable tools into the owning repo, not a shared location.
...
* Step 3: Executing Tools & Self-Correction (RLVR Loop):
   1. Retrieve relevant executable scripts or skills from the owning repo.
...
Execute this non-interactive bash sequence to bootstrap or audit the entire 5+1 Tier cognitive filesystem topology (applies to `data_vault/` in this repo — see 05_FEDERATED_FILE_TREE_TOPOLOGY_MASTER.md for where `data_vault/` sits in the federation):
...
mkdir -p 05_episodic_logs/{daily_driver_sync,trajectories,.incognito_red_sandbox,rlvr_verifiers,hygiene_reports}
...
- Red Audit Sandbox: `05_episodic_logs/.incognito_red_sandbox/`
...
*Source: Google Drive `__NovÆxorpus_LIVING_MASTER_CANON/00_DEFINITIVE_MASTER_SPECIFICATION_V3_COMPLETE.md.txt` (id `1_VgmRuvoru6wP0W0t_Td_oatgLgTZvbz`), read in full and placed 2026-09-07. Two `NOTE (repo correction)` blocks above are additions marking where this document's own text conflicts with corrections already decided this session — see `project_novae_xorpus_repo_layout_correction.md` and `project_red_auditor_not_detached.md`.*
