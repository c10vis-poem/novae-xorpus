...
**Not wired yet.** The vault at `/storage/emulated/0/OBSIDIAN_VAULT/OBSIDIAN_VAULT.md/`
has no `.git` and no `.obsidian/plugins/`. Nothing in it has ever synced, which
is why three handoffs written in August are still sitting there having never
reached GitHub. That's step zero.
